...
class sr_post(sr_instances):
    ...
    def help(self):
        ...
        print("-h|--help\n")
        print(
            "-blocksize [0|1|sz]        0-computed blocksize (default), 1-whole files (no partitioning), sz-fixed blocksize")
    ...
    def __on_post__(self):
        """ Post the current message and increment the count of publications attempted so far.
    ...
    def post_file(self, path, lstat, key=None, src_path=None):
        """ Decides whether a file needs to be posted as a part, as multiple parts or as a whole.

        :param path: the file path to be post
        :param lstat: the result of os.stat
        :param key: oldname provided if the file is moving
        :param src_path: file path of the source if the file is moving
        :return: True is the post was successful, False otherwise
        """
        self.logger.debug("post_file %s" % path )

        if self.path_rejected(path):
            return False
        if path.endswith('.' + self.msg.part_ext):
            ...
            # This variable means that part_file_assemble plugin is loaded and will handle posting the original file
            # (being assembled)
            return False
        ...
        if key is not None:
            # key is used when moving a file
            self.msg.headers[key] = src_path
            if key == 'oldname' and self.post_base_dir:
                self.msg.headers[key] = src_path.replace(self.post_base_dir, '')

        return self.__on_post__()
    ...
    def post_file_in_parts(self, path, lstat):
        ...
        for i in blocks:
            ...
            # all done, completing headers with results and posting
            self.msg.headers['parts'] = partstr
            self.msg.headers['sum'] = sumstr
            ok = self.__on_post__()
            if not ok:
    ...
    def post_file_part(self, path, lstat):
        """ Post a file when it was already identified as a part

        :param path: file path of the part file
        :param lstat: result of os.stat
        :return: True if the suffix, the on_part and the on_part were successful
        """
        self.post_init(path, lstat)
        ...
        if 'rename' in self.msg.headers and suffix not in self.msg.headers['rename']:
            # check rename, see if it has the right part suffix (if present)
            # TODO BL: I don't understand why we would need that here, if we add rename somewhere
            #  then it should put the part suffixe there
        ...
        # all checks done, we can now complete the message
        self.msg.headers['parts'] = partstr
        self.msg.headers['sum'] = sumstr
    ...
    def post_init(self, path, lstat=None, key=None, value=None):
        """ Initialize message with current path and predefined attributes

        :param path: path of the current post
        :param lstat: result of os.stat
        :param key: supplementary key to add to headers
        ...
        # Populate header with predefined attributes
        self.msg.headers = {}
        ...
        if self.source is not None:
            self.msg.headers['source'] = self.source
    ...
    def post1file(self, path, lstat):
        """ Post the file submitted whether it is a link, it gets deleted or it is a normal file.

        :param path: the file submitted
        :param lstat: result of os.stat
        :return: true if the operation is completed (not necessarily successful)
        """
        # FIXME BL post1file never returns the ok status only done (which is always true), this need to be reviewed:
        #  it seems to be related with the handling of the try except status which looks a bit wrong
        #  because it is bare and it doesn`t handle anything
        done = True
        ...
        if os.path.islink(path):
            ...
            if self.follow_symlinks:
                ...
                if os.path.exists(rpath):
                    lstat = os.stat(rpath)
                # FIXME BL a recursive call to follow symlinks seems quite dangerous,
                #  does realpath handle it properly ?
                ok = self.post1file(rpath, lstat)
            return done

        if lstat is None:
            # os.stat produce nothing: path deleted
    ...
    def process_event(self, event, src, dst):
        ...
        lstat = os.stat(src)
        if self.path_inflight(src, lstat):
            return later
    ...
    def set_blocksize(self, bssetting, fsiz):
        """ Compute the blocksize using a constant (or fsiz) if bssetting is not set.

        :param bssetting: switch to decides what to do if 0, 1 or other
        :param fsiz: the size of the file
        :return: the new block size (computed, fsiz or bssetting)
        """
