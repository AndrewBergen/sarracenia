...
class sr_config:
    ...
    def log_settings(self):
        ...
        self.logger.info(
            "\theartbeat=%s sanity_log_dead=%s default_mode=%03o default_mode_dir=%03o default_mode_log=%03o discard=%s durable=%s" % \
            (self.heartbeat, self.sanity_log_dead, self.chmod, self.chmod_dir, self.chmod_log, self.discard,
             self.durable))
        self.logger.info(
            "\tpost_on_start=%s preserve_mode=%s preserve_time=%s realpath_post=%s base_dir=%s follow_symlinks=%s" % \
            (self.post_on_start, self.preserve_mode, self.preserve_time, self.realpath_post, self.base_dir,
        ...
        if self.post_broker:
            self.logger.info(
                "\tpost_base_dir=%s post_base_url=%s post_topic_prefix=%s post_version=%s sum=%s blocksize=%s " % \
    ...
    def chunksize_from_str(self, str_value):
        ...
        if str_value[-1] in 'kK':
            factor = 1024
        if str_value[-1] in 'mM':
            factor = 1024 * 1024
        if str_value[-1] in 'gG':
            factor = 1024 * 1024 * 1024
        if str_value[-1] in 'tT':
    ...
    def duration_from_str(self, str_value, setting_units='s'):
        ...
        if setting_units[-1] == 's':
            ...
            if str_value[-1] in 'sS':
                factor *= 1
            elif str_value[-1] in 'mM':
                factor *= 60
            elif str_value[-1] in 'hH':
                factor *= 60 * 60
            elif str_value[-1] in 'dD':
                factor *= 60 * 60 * 24
            elif str_value[-1] in 'wW':
                factor *= 60 * 60 * 24 * 7
    ...
    def load_sums(self):
        ...
        for d in sumdirs:
            ...
            for s in os.listdir(d):
                ...
                try:
                    register_name = self.add_sumalgo.registered_as()
                except:
                    register_name = None
    ...
    def sundew_matchPattern(self, BN, EN, BP, keywd, defval):
        ...
        if keywd[:4] == "{T1}":
            return (EN[0])[0:1] + keywd[4:]
        elif keywd[:4] == "{T2}":
            return (EN[0])[1:2] + keywd[4:]
        elif keywd[:4] == "{A1}":
            return (EN[0])[2:3] + keywd[4:]
        elif keywd[:4] == "{A2}":
            return (EN[0])[3:4] + keywd[4:]
        elif keywd[:4] == "{ii}":
            return (EN[0])[4:6] + keywd[4:]
        elif keywd[:6] == "{CCCC}":
            return EN[1] + keywd[6:]
        elif keywd[:4] == "{YY}":
            return (EN[2])[0:2] + keywd[4:]
        elif keywd[:4] == "{GG}":
            return (EN[2])[2:4] + keywd[4:]
        elif keywd[:4] == "{Gg}":
            return (EN[2])[4:6] + keywd[4:]
        elif keywd[:5] == "{BBB}":
    ...
    def option(self, words):
        ...
        try:
            if words0 in ['accept', 'get', 'reject']:  # See: sr_config.7
                ...
                self.masks.append((
                                  pattern, self.currentDir, self.currentFileOption, mask_regexp, accepting, self.mirror,
                                  self.strip, self.pstrip, self.flatten))
            ...
            elif words0 in ['caching', 'cache', 'no_duplicates', 'noduplicates', 'nd', 'suppress_duplicates',
                            'sd']:  # See: sr_post.1 sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
                    self.caching = 300
                    n = 1
                else:
                ...
                if words1 in known_bases:
                    ...
                    self.logger.error(
                        "unknown basis for duplicate suppression: %s, should be one of: %s (default: %s)" % \
            ...
            # adding paths in command line might be a mess...
                ...
                if not ok:
                    self.logger.error("unknown checksum specified: %s, should be one of: %s or z" % (
                    self.sumflg, ', '.join(self.sumalgos.keys())))
                    needexit = True
                n = 2
                ...
                if words1 in known_runs:
                    ...
                    self.logger.error(
                        "unknown choice of what to run on Windows: %s, should be one of: %s (default: %s)" % \
    ...
    def validate_parts(self):
        ...
        if len(token) > 1:
            ...
            if self.partflg == 'p':
                self.logger.error(
                    "parts invalid strategy p arguments partial file posting not supported (%s)" % self.parts)
                return False
            ...
            if (self.partflg == 'i' or self.partflg == '0'):
                ...
                try:
                    self.blocksize = self.chunksize_from_str(token[1])
                except:
