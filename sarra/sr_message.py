...
class sr_message():
        ...
        if msg:
            ...
            if msg.body[0] == '[':
                ...
                if "integrity" in self.headers.keys():
                    sum_algo_v3tov2 = {"arbitrary": "a", "md5": "d", "sha512": "s", "md5name": "n", "random": "0",
                                       "link": "L", "remove": "R", "cod": "z"}
                    ...
                    if sa in ['0']:
                        ...
                        sv = encode(decode(self.headers["integrity"]["value"].encode('utf-8'), "base64"), 'hex').decode(
                            'utf-8')
                    # set event.
                    if sa == 'L':
                        self.event = 'link'
    ...
    def parse_v02_post(self):
        ...
        if self.mtype == 'report' or self.mtype == 'log':  # log included for compatibility... prior to rename..
            ...
            if self.version == 'v02':
                ...
                (self.report_elapse, self.report_code, self.report_host, self.report_user) = self.headers[
                    'report'].split()
    ...
    def post(self, parent):
        ...
        if self.post_exchange_split > 0:
            if 'integrity' in self.headers:
                ...
                self.logger.debug(
                    "post_exchange_split set, keying on %s , suffix is %s" % (self.headers['sum'][-1], suffix))
        else:
            suffix = ""
        ...
        if self.publisher != None:
            if self.post_version == 'v03':
                ...
                sum_algo_map = {"a": "arbitrary", "d": "md5", "s": "sha512", "n": "md5name", "0": "random", "L": "link",
                                "R": "remove", "z": "cod"}
                if 'sum' in self.headers:
                ...
                for h in self.headers:
                    ...
                    if len(self.headers[h].encode("utf8")) >= amqp_ss_maxlen:
                        ...
                        self.logger.warning(
                            "truncating %s header at %d characters (to fit 255 byte AMQP limit) to: %s " % \
    ...
    def verify_part_suffix(self, filepath):
        """ Parses the part suffix with expression .part.bsz,bno and stores info into corresponding attributes

        :param filepath: the path of the part file
        :return: True if the parse was successful, False otherwise
        """
        # TODO Actually it parses .bsz.blktot.brem.bno.sumflg.part this is something to fix/change at issue #201
        # TODO It should be renamed from verity_part_suffix to parse_part_suffix, because it is what it does
        ...
        try:
            ...
            self.logger.error("sr_message/verify_part_suffix: incorrect extension, {}".format(err))
        ...
        try:
            ...
            with open(filepath, 'rb') as fp:
                for i in range(fsiz):
                    ...
                    i = bufsum_size
            if bufsum_size != fsiz:
                self.logger.warning("sr_message verify_part_suffix incomplete reading %d %d" % (bufsum_size, fsiz))
                return False, 'missing data from file', None, None, None
            self.checksum = self.sumalgo.get_value()

            self.partstr = 'p,%d,%d,%d,%d' % (self.chunksize, self.block_count, self.remainder, self.current_block)
            self.sumstr = '%s,%s' % (self.sumflg, self.checksum)
        except Exception as err:
            self.logger.error("sr_message/verify_part_suffix: incorrect checksum, {}".format(err))
            self.logger.debug('Exception details: ', exc_info=True)
            return False, 'incorrect checksum', None, None, None
